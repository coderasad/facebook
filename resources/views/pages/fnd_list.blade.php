                    ...
                    <div class="col-12">
                        <form action="{{ url('fnd-list') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" placeholder="Victim Name" name="name" autocomplete="off"
                                       class="ar_form_control">
                                <label for="img">Victim Image</label>
                                <input type="file" name="victim_img" multiple class="form-control mb-4" id="img">
                                <label for="img">Friend Image</label>
                    ...
                    <div class="col-12">
                        <table class="table table-bordered">
                            @foreach ($fndlist as $data)
                                <tr>
                                    <td>{{$data->name}}</td>
                                    <td><span><img class="w-50" src="{{ asset($data->victim_img) }}" alt=""></span></td>
                                    @foreach (json_decode($data->img) as $item)

                                        <td><span><img class="w-50" src="{{ asset($item) }}" alt=""></span></td>

                                    @endforeach
                                    <td><a class="btn btn-danger" href="{{ Route('fnd-delete', $data->id) }}">Delete</a>
                                    </td>
                                </tr>
                            @endforeach
                        </table>
